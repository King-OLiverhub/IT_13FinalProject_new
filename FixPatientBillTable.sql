-- Fix patient_bill table for archive functionality
...
BEGIN
    ...
    -- Drop foreign key constraint if it causes issues
    IF EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_patient_bill_patients')
    BEGIN
        ALTER TABLE patient_bill DROP CONSTRAINT FK_patient_bill_patients;
        PRINT 'Dropped FK_patient_bill_patients constraint';
    END
    
    PRINT 'patient_bill table updated successfully!';
END
ELSE
BEGIN
    PRINT 'patient_bill table does not exist';
END
